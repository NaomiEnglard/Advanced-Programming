...
public class ticTacToe {
	...
	public static void Turn(String[][] board, String whoseTurn) {
		...
		boolean retry = true; // if there is a mistake allow user to re-enter
								// the spot he wants to go in
		while (retry) { // keep looping if user enters a entry is invalid		
			retry = false;// assume there are no errors, if there are then the
							// erroe will turn retry to true
			try {
				System.out.print("choose a row: 1,2, or 3 for player "
						+ whoseTurn);
				rowChoice = choice.nextInt();
				System.out.print("choose a colum: 1,2, or 3 for player "
						+ whoseTurn);
				columChoice = choice.nextInt();

				// only allow someone to go in a spot that no one else is in
				if ((board[rowChoice - 1][columChoice - 1]
						.equalsIgnoreCase(" "))) {
					// an x or o should be filled into the user's choice
					board[rowChoice - 1][columChoice - 1] = whoseTurn;
				} else { // if there is a marker in the spot error there is someone already there allow them
							// to read in again
					//throw new CantGoThereException();
					System.out.println("there is a marker in that spot please choose another spot");
					retry=true;	//allow user to re-enter a location for his marker
				}
			} catch (ArrayIndexOutOfBoundsException ex) {
				System.out
						.println("Enter a number between 1-3 for the rows and colums value");
				retry = true;//allow user to re-enter a location for his marker
			}
			catch(InputMismatchException ex1){
				System.out.println("you did not enter a number the game is ending");
				System.exit(0);
			}
		}
