...
public class Account {
	...
	public Account(int userId, double userBalance){
		//if(userId<lastID)//if they enter an id less than 0, or an id that an account has a new account should not be created
	...
	private void setId(int id) {//the user should not be able to set the id of his account
		this.id = id;
	}
	public Calendar getDateCreated() {
		GregorianCalendar sender = new GregorianCalendar();
		sender.set(this.dateCreated.get(Calendar.YEAR), this.dateCreated.get(Calendar.MONTH), this.dateCreated.get(Calendar.DAY_OF_MONTH));
		return sender;
	}
	...
	public double  getBalanceAsOf(GregorianCalendar date) {
		...
		//add the deposits to the initial Balance if they are before or equal to the date provided
		for(Deposit payment: depositList){
			if(date.before(payment.getDayOfDeposit())||date.equals(payment.getDayOfDeposit())){
				totalAsOfDate+=payment.getAmountDeposited();
			}
		}//remove withdrawls from the total balance if they are made before at the time provided
		for(Withdrawl debt: withdrawlList){
			if(date.before(debt.getDayOfWithdrawl())||date.equals(debt.getDayOfWithdrawl())){
	...
	public void setAnnualInterestRate(double annualInterestRate) {
		//although a user should not be able to set his Interst Rate, this progam requires it so it is public method
		this.annualInterestRate = annualInterestRate;
	}
	//method to return monthly intrest rate
	public double getMonthlyInterstRate(){
		double MonthlyIR = (this.annualInterestRate/120);	//devide by 120 b/c u need to devide by 12 for monthly rate and by 100 to get rid of the percent	
		return MonthlyIR;
	}
	...
	public double withdraw(double amountToWithdraw)throws InsufficientFundsException{
		//this.balance-=amountToWithdraw;
		if (amountToWithdraw<0 || amountToWithdraw>this.getBalance())
			//if the user puts in a negative number or a withdraw amount greater than their balance thorw error
	...
	public String printStatement(GregorianCalendar startdate, GregorianCalendar endDate){
		...
		//display each deposit that occurred from start date to end date = after start date and beofore or at the time of the end date	
		for(Deposit input: depositList){
			holder= input.getDayOfDeposit();
			if(holder.after(startdate)&& (holder.before(endDate)|| holder.equals(endDate))){
	...
	public ArrayList<Deposit> allDepposits(){
		...
		for(Deposit rewrite: this.depositList){
			list.add(new Deposit(rewrite.getAmountDeposited(),rewrite.getDayOfDeposit()));
		}
		return list;
	}
	public ArrayList<Withdrawl> allWithdrawls(){
		...
		for(Withdrawl rewrite: this.withdrawlList){
			list.add(new Withdrawl(rewrite.getAmountWithdrawl(),rewrite.getDayOfWithdrawl()));
		}
		return list;
	}
	public Double getLargestDeposit(){ //throws  NullPointerException{
		...
		if(biggest==-1){ //if no deposits where made the answer will be null
			biggest=null;
		}
		return biggest ;
	...
	public String toString(){
		...
		buffer.append("\tThe account was set up on day: " + formatter.format(this.dateCreated.getTime()));
